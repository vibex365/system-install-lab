...
export default function Apply() {
  ...
  const form = useForm<ApplyValues>({
    ...
    defaultValues: { name: "", email: "", product: "", bottleneck: "", whyNow: "" },
  });

  const onSubmit = (data: ApplyValues) => {
    const entry = { ...data, id: crypto.randomUUID(), createdAt: new Date().toISOString() };
    const prev = JSON.parse(localStorage.getItem("pfsw_apply_submissions") || "[]");
    localStorage.setItem("pfsw_apply_submissions", JSON.stringify([...prev, entry]));
          ...
          {submitted ? (
            <div className="max-w-md mx-auto text-center">
              <div className="rounded-2xl border border-primary/30 bg-card p-10">
                ...
                <p className="text-sm text-muted-foreground mb-6">We'll be in touch within 48 hours.</p>
                <div className="flex flex-col gap-3">
                  <Button asChild variant="outline" className="border-primary/30 text-foreground hover:bg-primary/10">
                    <Link to="/">Back to Home</Link>
                  </Button>
                  <Button asChild variant="ghost" size="sm">
                    <Link to="/waitlist">Join the waitlist anyway <ArrowRight className="ml-1 h-3 w-3" /></Link>
                  </Button>
                </div>
              </div>
            </div>
                ...
                <div className="rounded-2xl border border-border bg-card p-6 md:p-8">
                  <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
                      ...
                      <Button type="submit" size="lg" className="w-full tracking-wide font-bold" disabled={form.formState.isSubmitting}>
                        Submit Application
                      </Button>
                    </form>
                  </Form>
