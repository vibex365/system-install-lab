...
function Dream100Content() {
  ...
  const [outreachEntry, setOutreachEntry] = useState<Dream100Entry | null>(null);
  ...
  const openOutreach = (entry: Dream100Entry) => {
    setOutreachEntry(entry);
    setOutreachChannel(entry.email ? "email" : "sms");
    setOutreachOpen(true);
  };

  const sendOutreach = async () => {
    if (!user || !outreachEntry) return;
    setOutreachSending(true);
    try {
      const { data, error } = await supabase.functions.invoke("agent-dream100-outreach", {
        body: { entry_id: outreachEntry.id, channel: outreachChannel, user_id: user.id },
      });
      if (error) throw error;
      toast({ title: "Outreach sent!", description: `${outreachChannel === "email" ? "Email" : "SMS"} sent to ${outreachEntry.name}` });
                      ...
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="flex items-center gap-1 shrink-0">
                          {(entry.email || entry.phone) && (
                            <button onClick={() => openOutreach(entry)} className="p-1.5 text-muted-foreground hover:text-primary transition-colors" title="Send Outreach">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input value={formEmail} onChange={(e) => setFormEmail(e.target.value)} placeholder="email@example.com" className="h-9 text-xs" />
              </div>
              <div>
                <Label className="text-xs">Phone</Label>
                <Input value={formPhone} onChange={(e) => setFormPhone(e.target.value)} placeholder="+1..." className="h-9 text-xs" />
              </div>
            </div>
            <div>
        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="text-base flex items-center gap-2">
              ...
              Send Outreach to {outreachEntry?.name}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <div>
              <Label className="text-xs">Channel</Label>
              <Select value={outreachChannel} onValueChange={(v) => setOutreachChannel(v as "email" | "sms")}>
                <SelectTrigger className="h-9 text-xs"><SelectValue /></SelectTrigger>
                <SelectContent>
                  {outreachEntry?.email && <SelectItem value="email" className="text-xs"><span className="flex items-center gap-1"><Mail className="h-3 w-3" /> Email ({outreachEntry.email})</span></SelectItem>}
                  {outreachEntry?.phone && <SelectItem value="sms" className="text-xs"><span className="flex items-center gap-1"><Phone className="h-3 w-3" /> SMS ({outreachEntry.phone})</span></SelectItem>}
                </SelectContent>
              </Select>
            </div>
            <p className="text-[11px] text-muted-foreground">
              AI will generate a personalized partnership pitch based on their profile, platform, and niche — then send it automatically.
            </p>
            <Button onClick={sendOutreach} disabled={outreachSending} className="w-full">
              {outreachSending ? <Loader2 className="h-3 w-3 animate-spin mr-1" /> : <Send className="h-3 w-3 mr-1" />}
              Generate & Send {outreachChannel === "email" ? "Email" : "SMS"}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
